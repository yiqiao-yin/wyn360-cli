...
"""
    ...
    async def chat_stream(self, user_message: str):
        ...
        This uses pydantic-ai's run_stream() which DOES execute tools properly,
        and streams text as it's generated by the model.

        Args:
            user_message: The user's input message

        Yields:
            Chunks of text as they're generated (accumulated, not deltas)
        """
        try:
            # Add user message to history
            self.conversation_history.append({
            ...
            if code_blocks:
                ...
                if python_blocks and is_blank_project():
                    ...
                    if success:
                        save_message = f"\n\n✓ Code saved to: {filename}"
                        # Yield final text with save message
                        yield response_text + save_message
