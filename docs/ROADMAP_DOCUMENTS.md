...
#### Tasks:
- [ ] Install optional dependency: `python-docx`
- [ ] Implement `WordReader` class
  - [ ] Open .docx files
  ...
  - [ ] Handle images:
    - [ ] Default: "describe" mode (extract alt text/captions)
    - [ ] Optional: "skip" mode (ignore images)
    - [ ] Optional: "vision" mode (use Claude vision API)
  - [ ] Preserve document hierarchy
- [ ] Structure-aware chunking:
  - [ ] Chunk by major sections (H1/H2 boundaries)
  - [ ] Keep tables within chunks (don't split)
  - [ ] Target ~1000 tokens per chunk
  - [ ] Preserve section context in chunk metadata
- [ ] Register `read_word` tool in `agent.py`:
  ```python
  @agent.tool
  async def read_word(
      file_path: str,
  ...
  ```
- [ ] Add `/set_image_mode <skip|describe|vision>` slash command
- [ ] Image handling implementation:
  - [ ] "skip": Ignore images entirely
  - [ ] "describe": Extract alt text, captions, nearby text
  - [ ] "vision": Call Claude vision API (warn about costs)
- [ ] Output format:
  - [ ] Document header (pages, sections, tokens)
  - [ ] Section-by-section summaries
  - [ ] Tables preserved in markdown
  - [ ] Image handling indicators
- [ ] Error handling:
  - [ ] File not found
